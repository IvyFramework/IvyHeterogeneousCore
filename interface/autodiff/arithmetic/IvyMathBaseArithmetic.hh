...
namespace IvyMath{
  ...
  */
  ...
  public:
    __CUDA_HOST__ IvyRegularFunction_1D(IvyThreadSafePtr_t<T> const& dep);
    __CUDA_HOST__ IvyRegularFunction_1D(IvyRegularFunction_1D const& other);
    __CUDA_HOST__ IvyRegularFunction_1D(IvyRegularFunction_1D&& other);
  ...
  */
  ...
  public:
    __CUDA_HOST__ IvyRegularFunction_2D(IvyThreadSafePtr_t<T> const& x, IvyThreadSafePtr_t<U> const& y);
    __CUDA_HOST__ IvyRegularFunction_2D(IvyRegularFunction_2D const& other);
    __CUDA_HOST__ IvyRegularFunction_2D(IvyRegularFunction_2D&& other);
  ...
  template<typename T> struct SqrtFcnal<T, real_domain_tag>{
    ...
    using grad_t = IvyThreadSafePtr_t<IvyFunction<value_t, real_domain_tag>>;
    static __CUDA_HOST_DEVICE__ value_t eval(T const& x);
    static __CUDA_HOST_DEVICE__ grad_t gradient(IvyThreadSafePtr_t<T> const& x);
  };
  ...
  template<typename T, typename U, ENABLE_IF_BOOL(!is_pointer_v<T> && !is_pointer_v<U>)>
  __INLINE_FCN_FORCE__ __CUDA_HOST_DEVICE__ typename PowFcnal<T, U>::value_t Pow(T const& x, U const& y);
  template<typename T, typename U, ENABLE_IF_BOOL(is_pointer_v<T>&& is_pointer_v<U>)>
