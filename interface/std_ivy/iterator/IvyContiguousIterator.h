...
namespace std_ivy{
  ...
  public:
    __HOST_DEVICE__ void set_mem_loc(pointer const& mem_loc, IvyMemoryType mem_type, IvyGPUStream* stream){
    ...
    __HOST_DEVICE__ mem_loc_t get_mem_loc() const{
      constexpr IvyMemoryType def_mem_type = IvyMemoryHelpers::get_execution_default_memory();
    ...
    __HOST_DEVICE__ mem_loc_t& get_mem_loc_fast(){ return *ptr_mem_loc_; }
    __HOST_DEVICE__ mem_loc_t const& get_mem_loc_fast() const{ return *ptr_mem_loc_; }

    __HOST_DEVICE__ mem_loc_container_t& get_ptr_mem_loc(){ return ptr_mem_loc_; }
    __HOST_DEVICE__ mem_loc_container_t const& get_ptr_mem_loc() const{ return ptr_mem_loc_; }

    __HOST_DEVICE__ IvyContiguousIterator() : next_(nullptr), prev_(nullptr){}
    __HOST_DEVICE__ IvyContiguousIterator(IvyContiguousIterator const& other) :
      ptr_mem_loc_(other.ptr_mem_loc_), next_(other.next_), prev_(other.prev_)
    {}
    __HOST_DEVICE__ IvyContiguousIterator(IvyContiguousIterator&& other) :
    ...
    __HOST_DEVICE__ IvyContiguousIterator(pointer const& mem_loc, IvyMemoryType mem_type, IvyGPUStream* stream) :
      next_(nullptr), prev_(nullptr)
    {
      this->set_mem_loc(mem_loc, mem_type, stream);
    }
    __HOST_DEVICE__ ~IvyContiguousIterator(){}

    __INLINE_FCN_FORCE__ __HOST_DEVICE__ pointable_t& next(){ return next_; }
    __INLINE_FCN_FORCE__ __HOST_DEVICE__ pointable_t& prev(){ return prev_; }
    __INLINE_FCN_FORCE__ __HOST_DEVICE__ pointable_t next() const{ return next_; }
    __INLINE_FCN_FORCE__ __HOST_DEVICE__ pointable_t prev() const{ return prev_; }

    __HOST_DEVICE__ IvyContiguousIterator& operator=(IvyContiguousIterator const& other){
      ptr_mem_loc_ = other.ptr_mem_loc_;
      next_ = other.next_;
      prev_ = other.prev_;
      return *this;
    }
    __HOST_DEVICE__ IvyContiguousIterator& operator=(IvyContiguousIterator&& other){
    ...
    __HOST_DEVICE__ void swap(IvyContiguousIterator& other){
      std_util::swap(ptr_mem_loc_, other.ptr_mem_loc_);
      std_util::swap(next_, other.next_);
      std_util::swap(prev_, other.prev_);
    }

    __HOST_DEVICE__ reference operator*() const{ return *(this->get_mem_loc_fast()); }
    __HOST_DEVICE__ pointer const& operator->() const{ return this->get_mem_loc_fast(); }
    __HOST_DEVICE__ bool is_valid() const{ return (ptr_mem_loc_ && this->get_mem_loc()); }

    __HOST_DEVICE__ IvyContiguousIterator& operator++(){
      *this = *(this->next());
      return *this;
    }
    __HOST_DEVICE__ IvyContiguousIterator operator++(int){ IvyContiguousIterator tmp(*this); operator++(); return tmp; }
    __HOST_DEVICE__ IvyContiguousIterator& operator--(){
      *this = *(this->prev());
      return *this;
    }
    ...
    __HOST_DEVICE__ IvyContiguousIterator operator+(difference_type n) const{
      if (n==0) return *this;
    ...
    __HOST_DEVICE__ IvyContiguousIterator& operator+=(difference_type n){ *this = *this + n; return *this; }
    __HOST_DEVICE__ IvyContiguousIterator& operator-=(difference_type n){ *this = *this - n; return *this; }

    __HOST_DEVICE__ bool operator==(IvyContiguousIterator const& other) const{
      return
        (ptr_mem_loc_ == other.ptr_mem_loc_)
        &&
        (!ptr_mem_loc_ || this->get_mem_loc()==other.get_mem_loc());
    }
    __HOST_DEVICE__ bool operator!=(IvyContiguousIterator const& other) const{ return !(*this==other); }

    __HOST_DEVICE__ difference_type operator-(IvyContiguousIterator const& other) const{
